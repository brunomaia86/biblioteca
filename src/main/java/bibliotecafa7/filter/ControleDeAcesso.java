...
public class ControleDeAcesso implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse respose, FilterChain chain)
		...
		if (session.getAttribute("usuario") != null || req.getRequestURI().endsWith("login.xhtml")
				|| req.getRequestURI().endsWith("/usuario/formulario-usuario.xhtml")) {
			chain.doFilter(request, respose);
		} else {
			HttpServletResponse res = (HttpServletResponse) respose;
			res.sendRedirect("/bibliotecafa7/login.xhtml");
		}
	}
